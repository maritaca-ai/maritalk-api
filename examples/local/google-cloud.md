...
Este tutorial mostra como executar a MariTalk Local Small na Google Cloud Platform (GCP). Para isso, utilizaremos uma instância com a GPU NVIDIA A100 40GB.
...
Para a versão MariTalk-small rodando em uma máquina com 1xA100 40GB, é esperado um throughput de ~55 tokens/s, considerando uma entrada de 1.000 tokens e saída de 500 tokens. Ou seja, uma requisição com esse tamanho de entrada e saída vai demorar cerca de 27 segundos para finalizar. Conforme o número de requisições simultâneas aumenta, é esperado ver um aumento no throughput total do sistema, mas uma queda do ponto de vista de cada requisição isoladamente.

Disponibilizamos [esta ferramenta de benchmark](https://github.com/maritaca-ai/maritalk-api/blob/main/examples/local/benchmark.py) para que você possa avaliar o desempenho no seu ambiente.
